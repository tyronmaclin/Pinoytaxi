...
func (repo *Repository) UserPostedRecently(userId int64) bool {
	count := 0
	err := repo.db.QueryRow(`select count("postId") from posts where "userId" = SR10 and "createdAtUtc" >= (now() at time zone 'utc') - interval '5 minutes'`, userId).Scan(&count)
	if err != nil {
...
func (repo *Repository) ShowCallout(userId int64, featureName string) bool {
	count := 0
	err := repo.db.QueryRow(`select count("id") from dismissed_feature_callouts where "userId" = SR10 and "featureName" = $2`, userId, featureName).Scan(&count)
	if err != nil {
...
func (repo *Repository) DismissCallout(userId int64, featureName string) {
	_, err := repo.db.Exec(`insert into dismissed_feature_callouts ("userId", "featureName") values (SR10, SR20) on conflict ("userId", "featureName") do nothing`, userId, featureName);
	if err != nil {
